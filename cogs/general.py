...
### GROUP COMMAND JEUX START ###
    @commands.group(name="jeux", pass_context=True)
	...
	#TODO: Ameliorer ce jeu basique
    @_games.command(pass_context=True)
    async def roll(self, ctx, number : int = 100):
        ...
        if number > 1:
            ...
            await self.bot.say("{} Peut-être plus grand que 1 ? ;P".format(author.mention))
			
	#TODO: REGLES DU 421 : https://fr.wikipedia.org/wiki/421_(jeu)
    @_games.command(name="421", aliases=["zanzibar"], pass_context=True)
    async def _zanzibar(self, ctx, number : int = 100):
        """Rolls random number (between 1 and user choice)
    ...
    @_games.command(pass_context=True)
    async def pileouface(self, ctx): #async def pileouface(self, ctx, user : discord.Member=None):
        """Pile ou Face ?"""
        # if user != None:
            # msg = ""
        ...
        # else:
        await self.bot.say("*Virevolte et ... " + choice(["PILE!*", "FACE!*"]))
    ...
    @_games.command(name="8", aliases=["8ball"])
    ...
    async def auproprio(self, ctx, *, message: str):
    ...
    @commands.command()
    async def urban(self, *, search_terms : str, definition_number : int=1): #TODO: See if it is possible to access a french dictionnary
        """Urban Dictionary search in english
